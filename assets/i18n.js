...
const i18nTexts = {
    ...
    'results-count-pattern': '为 "${query}" 找到 ${count} 个结果',
        ...
        'stylecool-badge': '设计模式接口',
        'stylecool-header-subtitle': '给 AI 与自动化工作流的设计模式 StyleCool — 把输出折成输入、命中 Prompt 缓存',
        'stylecool-hero-badge': 'STYLE COOL',
        'stylecool-hero-title': '设计模式索引',
        'stylecool-hero-desc': '面向 AI 编程助手的<strong>设计知识 JSON 接口</strong>。<br>按平台分类检索设计规则、组件模式与反模式，让 AI 据此生成高质量 UI，把昂贵的输出 token 折成便宜的输入 token。',
        'stylecool-value-tokens-title': '把输出折成输入',
        'stylecool-value-tokens-desc': 'AI 先查设计模式索引取回规则（name / 描述 / CSS / 禁止项），再据此生成 UI。模型不必从零「编造」样式——输出 token 折算为便宜的输入 token。',
        'stylecool-value-platform-title': '平台专项检索',
        'stylecool-value-platform-desc': '按 target 分类（网站 / 桌面 / 小程序 / 手机），每次查询只返回最相关的平台规则，不往上下文灌无关的样式噪音。',
        ...
        'stylecool-api-response-desc': '返回：JSON — <code>results</code>（设计模式数组）、<code>query</code>、<code>cat</code>、<code>count</code>',
        ...
        'stylecool-quicktip-zh': '【中文】 向以下地址发送 GET 请求，取回设计模式索引：',
        'stylecool-quicktip-en': '[English] GET https://style.functioncool.xyz/skillapi?token=mutantcat&q=button&cat=web',
        'stylecool-repo-title': '设计模式 JSON 开源',
        'stylecool-repo-desc': '欢迎扩充条目，在以下仓库提交 PR：',
        'stylecool-back-link': '← 返回 FunctionCool',
    ...
    'results-count-pattern': 'Found ${count} results for "${query}"',
        ...
        // StyleCool page
        'stylecool-title': 'StyleCool',
        'stylecool-badge': 'Design Pattern API',
        'stylecool-header-subtitle': 'A Design Pattern Skill for AI agents & automation — turn output tokens into input tokens, hit the prompt cache',
        'stylecool-hero-badge': 'STYLE COOL',
        'stylecool-hero-title': 'Design Pattern Index',
        'stylecool-hero-desc': 'A <strong>design knowledge JSON API</strong> for AI coding assistants.<br>Search design rules, component patterns and anti-patterns by platform category — let AI generate high-quality UI while turning expensive output tokens into cheap input tokens.',
        'stylecool-value-tokens-title': 'Trade output for input tokens',
        'stylecool-value-tokens-desc': 'AI first queries the design pattern index to retrieve rules (name / description / CSS / anti-patterns), then generates UI from them. The model never "invents" styles from scratch — expensive output tokens become much cheaper input tokens.',
        'stylecool-value-platform-title': 'Platform-specific search',
        'stylecool-value-platform-desc': 'Categorized by target platform (Web / Desktop / Mini Program / Mobile) — each query returns only the most relevant platform rules, without flooding the context with irrelevant style noise.',
        ...
        'stylecool-api-response-desc': 'Response: JSON — <code>results</code> (design pattern array), <code>query</code>, <code>cat</code>, <code>count</code>',
        ...
        'stylecool-quicktip-zh': '【中文】 Send a GET request to the address below to fetch the design pattern index:',
        'stylecool-quicktip-en': '[English] GET https://style.functioncool.xyz/skillapi?token=mutantcat&q=button&cat=web',
        'stylecool-repo-title': 'Design Pattern JSON — Open Source',
        'stylecool-repo-desc': 'Contributions welcome — submit a PR at:',
        'stylecool-back-link': '← Back to FunctionCool',
